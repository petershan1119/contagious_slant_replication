...
reghdfe St_rtgxx_county_fxnc $covs_base [aweight=circ] , absorb(fips_state) cluster(fips_state id_news),  if in_main
predict St_rtgxx_county_fxnc_pre if e(sample)
reghdfe St_rtgxx_county_fxnc_pre St_pos_fxnc_county_repl [aweight=circ] , absorb(fips_state) cluster(fips_state id_news)
eststo col1

reghdfe St_percent_fox $covs_base [aweight=circ] , absorb(fips_state) cluster(fips_state id_news),  if in_main
predict St_percent_fox_pre if e(sample)
reghdfe St_percent_fox_pre St_pos_fxnc_county_repl [aweight=circ] , absorb(fips_state) cluster(fips_state id_news)
eststo col2

/*
*fox msnbc
ivreghdfe St_percent_fox (St_cons_fox_msnbc=St_instr_fox_msnbc) share_cnn share_fxnc share_mnbc county_pop $covs_base pos_cnn_county_repl [aweight=circ] , absorb(fips_state) partial( share_cnn share_fxnc share_mnbc $covs_base) cluster(fips_state id_news),  if St_instr_fox_msnbc!=.
eststo

reghdfe St_cons_fox_msnbc $covs_base [aweight=circ] , absorb(fips_state) cluster(fips_state id_news),  if _est_est4==1
predict St_cons_fox_msnbc_pre if e(sample)
reghdfe St_cons_fox_msnbc_pre St_instr_fox_msnbc [aweight=circ] , absorb(fips_state) cluster(fips_state id_news)
eststo col3

reghdfe St_percent_fox $covs_base [aweight=circ] , absorb(fips_state) cluster(fips_state id_news),  if _est_est4==1
predict St_percent_fox_pre2 if e(sample)
reghdfe St_percent_fox_pre2 St_instr_fox_msnbc [aweight=circ] , absorb(fips_state) cluster(fips_state id_news)
eststo col4
*/
...
reghdfe St_cons_fox_cnn_msnbc $covs_base [aweight=circ] , absorb(fips_state) cluster(fips_state id_news),  if in_main
predict St_cons_fox_cnn_msnbc_pre if e(sample)
reghdfe St_cons_fox_cnn_msnbc_pre St_instr_fox_cnn_msnbc [aweight=circ] , absorb(fips_state) cluster(fips_state id_news)
eststo col5

reghdfe St_percent_fox $covs_base [aweight=circ] , absorb(fips_state) cluster(fips_state id_news),  if in_main
predict St_percent_fox_pre3 if e(sample)
reghdfe St_percent_fox_pre3 St_instr_fox_cnn_msnbc [aweight=circ] , absorb(fips_state) cluster(fips_state id_news)
eststo col6


* esttab col1 col2 col3 col4 col5 col6 using "C:\Users\sergioga\Dropbox\cable_local_news\results\cheks.tex", replace keep(pos_fxnc_county_repl St_instr_fox_msnbc St_instr_fox_cnn_msnbc ) label order() collabels(, none) ml(,none) cells(b(star fmt (%9.3f)) se(par)) stats(ftest r2 N, fmt(%9.3f %9.3f %9.0g) labels("F-test" "R$^2$" "N observations")) starlevels(* 0.10 ** 0.05 *** 0.01) 
esttab col1 col2 col5 col6  using "results/checks.tex", replace keep(St_pos_fxnc_county_repl St_instr_fox_cnn_msnbc ) label order() collabels(, none) ml(,none) cells(b(star fmt (%9.3f)) se(par)) stats(ftest r2 N, fmt(%9.3f %9.3f %9.0g) labels("F-test" "R$^2$" "N observations")) starlevels(* 0.10 ** 0.05 *** 0.01) 
