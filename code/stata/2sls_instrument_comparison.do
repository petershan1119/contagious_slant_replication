...
local instr St_pos_fxnc_county_repl
* St_instr_fox_cnn_msnbc

egen total_circ_by_np = sum(circ), by(id_news)
egen `instr'_by_np = sum(`instr'*circ), by(id_news)
replace `instr'_by_np = `instr'_by_np/total_circ_by_np

egen total_circ_by_county = sum(circ), by(fips_county)
egen newspaper_instrument = sum(`instr'_by_np*circ), by(fips_county)
replace newspaper_instrument = newspaper_instrument/fips_county

duplicates drop fips_county, force

eststo clear
global controls $covs_base
reghdfe St_share_rep2008 `instr' newspaper_instrument $controls, absorb(fips_state) cluster(fips_state)
estadd scalar ftest=e(widstat)
eststo
...
estadd scalar ftest=e(widstat)
eststo
esttab, stats(ftest r2 N, fmt(%9.3f %9.3f %9.0g) labels("F-test" "R$^2$" "N observations")) starlevels(* 0.1 ** 0.05 *** 0.01) keep(`instr' newspaper_instrument) cells(b(star fmt (%9.3f)) se(par))
esttab using "results/main_`ext'_ instr_comparison.tex", replace keep(`instr' newspaper_instrument) label order() collabels(, none) ml(,none) cells(b(star fmt (%9.3f)) se(par)) stats(ftest r2 N, fmt(%9.3f %9.3f %9.0g) labels("F-test" "R$^2$" "N observations")) starlevels(* 0.10 ** 0.05 *** 0.01) 
