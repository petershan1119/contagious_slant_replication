...
import excel "data/regression_data/main_model_no_filtering_non_ap_regression_data.xls", sheet("Sheet1") firstrow
...
* Use non-AP variables
drop percent_fox no_articles
rename no_articles_non_ap no_articles
rename percent_fox_non_ap percent_fox
...
esttab using "results/main_language_`ext'.tex", replace keep(St_*) label order() collabels(, none) ml(,none) cells(b(star fmt (%9.3f)) se(par)) stats(ftest r2 N, fmt(%9.3f %9.3f %9.0g) labels("F-test" "R$^2$" "N observations")) starlevels(* 0.10 ** 0.05 *** 0.01) 
