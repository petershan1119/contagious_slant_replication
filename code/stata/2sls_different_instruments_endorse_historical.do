...
**************************************************
use "/Users/philine/Desktop/newslibrary_project/endorsements_matching/complete_source_list_merged_complete_from_leo.dta"
...
sort source_nb
save "data/endorsements_1996.dta", replace
...
local ext "lgr_endorse_historical"
...
gen circ_diff = (circ-circ_hist)/circ_hist

* Run regressions for different instruments
eststo clear
global controls $covs_base $channel
ivreghdfe circ_diff (St_cons_fox_cnn_msnbc=St_instr_fox_cnn_msnbc) $controls, absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main & endorse_dem1996 == 1
estadd scalar ftest=e(widstat)
eststo
ivreghdfe circ_diff (St_cons_fox_cnn_msnbc=St_instr_fox_cnn_msnbc) $controls, absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main & neverendorser_1996 == 1
estadd scalar ftest=e(widstat)
eststo
ivreghdfe circ_diff (St_cons_fox_cnn_msnbc=St_instr_fox_cnn_msnbc) $controls, absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main & endorse_rep1996 == 1
estadd scalar ftest=e(widstat)
eststo
