...
* Transform circulation to capture relative circulation
egen circ_total = sum(circ), by(id_news)
sum circ
sum circ_total
gen circ_rel = circ/circ_total
sum circ_rel
rename circ circ_abs
rename circ_rel circ
...
eststo clear
global controls $covs_base circ_abs
...
estadd scalar ftest=e(widstat)
eststo
global controls $covs_base $channel circ_abs
ivreghdfe St_percent_fox (St_cons_fox_cnn_msnbc=St_instr_fox_cnn_msnbc) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main
estadd scalar ftest=e(widstat)
eststo
global controls $covs_base $channel $language circ_abs
ivreghdfe St_percent_fox (St_cons_fox_cnn_msnbc=St_instr_fox_cnn_msnbc) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main
estadd scalar ftest=e(widstat)
eststo
