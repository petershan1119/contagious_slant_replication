...
* For old data, merge with Placebo

clear
cd /Users/philine/Dropbox/contagious_slant/
import excel "/Users/philine/Desktop/newslibrary_project/data/mic/regression_data_mic_jan20.xls", sheet("Sheet1") firstrow clear
rename source source_nb
keep percent_fox_95_96 source_nb
duplicates drop
duplicates drop source_nb, force
sort source_nb
save "data/regression_data_mic_jan20_PLACEBO_OLD.dta", replace
...
sort source_nb
merge source_nb using "data/regression_data_mic_jan20_PLACEBO_OLD.dta"
save "data/new_data_with_PLACEBO_OLD.dta", replace

**************************************************

* Now normal for new data

clear
cd /Users/philine/Dropbox/contagious_slant/

use "data/new_data_with_PLACEBO_OLD.dta"
* import excel "/Users/philine/Desktop/newslibrary_project/data/main_model_no_filtering_placebo_regression_data.xls", sheet("Sheet1") firstrow
...
local ext "lgr_placebo"
local ext2 "lgr_placebo_obs"
...
egen SD_percent_fox_95_96 = sd(percent_fox_95_96)
...
gen St_percent_fox_95_96 = percent_fox_95_96/SD_percent_fox_95_96
...
* Which obs are in Placebo?
capture confirm variable in_placebo
if !_rc {
drop in_placebo
}
eststo clear
global controls $covs_base $channel $language 
ivreghdfe St_percent_fox_95_96 (St_cons_fox_cnn_msnbc=St_instr_fox_cnn_msnbc) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars')
estadd scalar ftest=e(widstat)
eststo
rename _est_est1 in_placebo

* Run main regressions for different instruments, but only with Placebo obs
eststo clear
global controls $covs_base
ivreghdfe St_percent_fox (St_cons_fox_cnn_msnbc=St_instr_fox_cnn_msnbc) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_placebo
estadd scalar ftest=e(widstat)
eststo
...
esttab using "results/main_`ext2'_instr1.tex", replace keep(St_cons_fox_cnn_msnbc) label order() collabels(, none) ml(,none) cells(b(star fmt (%9.3f)) se(par)) stats(ftest r2 N, fmt(%9.3f %9.3f %9.0g) labels("F-test" "R$^2$" "N observations")) starlevels(* 0.10 ** 0.05 *** 0.01) 

eststo clear
global controls $covs_base
ivreghdfe St_percent_fox (St_cons_fox_cnn=St_instr_fox_cnn) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_placebo
estadd scalar ftest=e(widstat)
eststo
global controls $covs_base $channel
ivreghdfe St_percent_fox (St_cons_fox_cnn=St_instr_fox_cnn) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_placebo
...
esttab using "results/main_`ext2'_instr2.tex", replace keep(St_cons_fox_cnn) label order() collabels(, none) ml(,none) cells(b(star fmt (%9.3f)) se(par)) stats(ftest r2 N, fmt(%9.3f %9.3f %9.0g) labels("F-test" "R$^2$" "N observations")) starlevels(* 0.10 ** 0.05 *** 0.01) 


eststo clear
global controls $covs_base
ivreghdfe St_percent_fox (St_cons_fox_msnbc=St_instr_fox_msnbc) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_placebo
estadd scalar ftest=e(widstat)
eststo
global controls $covs_base $channel
ivreghdfe St_percent_fox (St_cons_fox_msnbc=St_instr_fox_msnbc) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_placebo
...
esttab using "results/main_`ext2'_instr3.tex", replace keep(St_cons_fox_msnbc) label order() collabels(, none) ml(,none) cells(b(star fmt (%9.3f)) se(par)) stats(ftest r2 N, fmt(%9.3f %9.3f %9.0g) labels("F-test" "R$^2$" "N observations")) starlevels(* 0.10 ** 0.05 *** 0.01) 


eststo clear
global controls $covs_base
ivreghdfe St_percent_fox (St_rtgxx_county_fxnc=St_pos_fxnc_county_repl) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_placebo
estadd scalar ftest=e(widstat)
eststo
global controls $covs_base $channel
ivreghdfe St_percent_fox (St_rtgxx_county_fxnc=St_pos_fxnc_county_repl) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_placebo
...
esttab using "results/main_`ext2'_instr4.tex", replace keep(St_rtgxx_county_fxnc) label order() collabels(, none) ml(,none) cells(b(star fmt (%9.3f)) se(par)) stats(ftest r2 N, fmt(%9.3f %9.3f %9.0g) labels("F-test" "R$^2$" "N observations")) starlevels(* 0.10 ** 0.05 *** 0.01) 

* Run Placebo regressions for different instruments

eststo clear
global controls $covs_base
ivreghdfe St_percent_fox_95_96 (St_cons_fox_cnn_msnbc=St_instr_fox_cnn_msnbc) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main
estadd scalar ftest=e(widstat)
eststo
global controls $covs_base $channel
ivreghdfe St_percent_fox_95_96 (St_cons_fox_cnn_msnbc=St_instr_fox_cnn_msnbc) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main
...
eststo clear
global controls $covs_base
ivreghdfe St_percent_fox_95_96 (St_cons_fox_cnn=St_instr_fox_cnn) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main
estadd scalar ftest=e(widstat)
eststo
global controls $covs_base $channel
ivreghdfe St_percent_fox_95_96 (St_cons_fox_cnn=St_instr_fox_cnn) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main
...
eststo clear
global controls $covs_base
ivreghdfe St_percent_fox_95_96 (St_cons_fox_msnbc=St_instr_fox_msnbc) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main
estadd scalar ftest=e(widstat)
eststo
global controls $covs_base $channel
ivreghdfe St_percent_fox_95_96 (St_cons_fox_msnbc=St_instr_fox_msnbc) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main
...
eststo clear
global controls $covs_base
ivreghdfe St_percent_fox_95_96 (St_rtgxx_county_fxnc=St_pos_fxnc_county_repl) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main
estadd scalar ftest=e(widstat)
eststo
global controls $covs_base $channel
ivreghdfe St_percent_fox_95_96 (St_rtgxx_county_fxnc=St_pos_fxnc_county_repl) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main
