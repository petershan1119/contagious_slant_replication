...
egen SD_share_rep2008 = sd(share_rep2008)
...
gen St_share_rep2008 = share_rep2008/SD_share_rep2008

bys fips_county: gen dup=_n

**************************************************
* Main table with instrument comparison
**************************************************
...
* Run regressions for all instruments
eststo clear
ivreghdfe St_share_rep2008 (St_cons_fox_cnn_msnbc=St_instr_fox_cnn_msnbc) $covs_base, absorb(fips_state) partial($covs_base) cluster(`cluster_vars'), if in_main
estadd scalar ftest=e(widstat)
eststo
ivreghdfe St_share_rep2008 (St_cons_fox_cnn=St_instr_fox_cnn) $covs_base, absorb(fips_state) partial($covs_base) cluster(`cluster_vars'), if in_main
estadd scalar ftest=e(widstat)
eststo
ivreghdfe St_share_rep2008 (St_cons_fox_msnbc=St_instr_fox_msnbc) $covs_base, absorb(fips_state) partial($covs_base) cluster(`cluster_vars'), if in_main
estadd scalar ftest=e(widstat)
eststo
ivreghdfe St_share_rep2008 (St_rtgxx_county_fxnc=St_pos_fxnc_county_repl) $covs_base, absorb(fips_state) partial($covs_base) cluster(`cluster_vars'), if in_main
estadd scalar ftest=e(widstat)
eststo
esttab, stats(ftest r2 N, fmt(%9.3f %9.3f %9.0g) labels("F-test" "R$^2$" "N observations")) starlevels(* 0.1 ** 0.05 *** 0.01) keep(St_cons_fox_cnn_msnbc St_cons_fox_cnn St_cons_fox_msnbc St_rtgxx_county_fxnc) cells(b(star fmt (%9.3f)) se(par))
esttab using "results/main_`ext'_all_instr.tex", replace keep(St_cons_fox_cnn_msnbc St_cons_fox_cnn St_cons_fox_msnbc St_rtgxx_county_fxnc) label order() collabels(, none) ml(,none) cells(b(star fmt (%9.3f)) se(par)) stats(ftest r2 N, fmt(%9.3f %9.3f %9.0g) labels("F-test" "R$^2$" "N observations")) starlevels(* 0.10 ** 0.05 *** 0.01) 
