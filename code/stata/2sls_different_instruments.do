...
* Plot distribution of prediction for newspapers
bysort id_news: gen dup_id_news = _n
hist percent_fox if in_main & dup_id_news == 1, lcolor(green) freq width(0.005) xtitle(Similarity to FNC (not standardized)) ytitle(Frequency (absolute)) lcolor(green) color(white)
graph export "figures/frequency_predictions_`ext'.png", replace
drop dup_id_news
