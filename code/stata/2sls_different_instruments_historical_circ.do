 ...
 * Choose dataset
* import excel "data/regression_data/main_model_with_filtering_regression_data.xls", sheet("Sheet1") firstrow
...
ivreghdfe circ_diff (St_cons_fox_cnn_msnbc=St_instr_fox_cnn_msnbc) $controls, absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main
estadd scalar ftest=e(widstat)
eststo
...
eststo clear
global controls $covs_base
ivreghdfe circ_diff (St_cons_fox_cnn=St_instr_fox_cnn) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main
estadd scalar ftest=e(widstat)
eststo
global controls $covs_base $channel
ivreghdfe circ_diff (St_cons_fox_cnn=St_instr_fox_cnn) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main
...
eststo clear
global controls $covs_base
ivreghdfe circ_diff (St_cons_fox_msnbc=St_instr_fox_msnbc) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main
estadd scalar ftest=e(widstat)
eststo
global controls $covs_base $channel
ivreghdfe circ_diff (St_cons_fox_msnbc=St_instr_fox_msnbc) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main
...
eststo clear
global controls $covs_base
ivreghdfe circ_diff (St_rtgxx_county_fxnc=St_pos_fxnc_county_repl) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main
estadd scalar ftest=e(widstat)
eststo
global controls $covs_base $channel
ivreghdfe circ_diff (St_rtgxx_county_fxnc=St_pos_fxnc_county_repl) $controls [aweight=circ], absorb(fips_state) partial($controls) cluster(`cluster_vars'), if in_main
