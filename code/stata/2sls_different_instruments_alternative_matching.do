...
import excel "/Users/philine/Desktop/newslibrary_project/data/alternative_matching_no_filtering_regression_data.xls", sheet("Sheet1") firstrow

rename source_nb source
rename n no_articles
gen fips_county=fips

* Define extension for files to be saved (lgr or deep)
local ext "lgr_alternative_matching"
local ext2 "lgr_alternative_matching_circ"

* Threshold for number of articles
drop if no_articles < 1000

* Define clustering
* local clustvar "fips_county"
local clustvar "fips_state"

* Newspapers are uniquely identified by: source
drop if mi(source)
...
ivreghdfe St_percent_fox (St_cons_fox_cnn_msnbc=St_instr_fox_cnn_msnbc) $controls, absorb(fips_state) partial($controls) cluster(`clustvar')
...
ivreghdfe St_percent_fox (St_cons_fox_cnn_msnbc=St_instr_fox_cnn_msnbc) $controls, absorb(fips_state) partial($controls) cluster(`clustvar'), if in_main
estadd scalar ftest=e(widstat)
eststo
...
eststo clear
global controls $covs_base
ivreghdfe St_percent_fox (St_cons_fox_cnn=St_instr_fox_cnn) $controls, absorb(fips_state) partial($controls) cluster(`clustvar'), if in_main
estadd scalar ftest=e(widstat)
eststo
global controls $covs_base $channel
ivreghdfe St_percent_fox (St_cons_fox_cnn=St_instr_fox_cnn) $controls, absorb(fips_state) partial($controls) cluster(`clustvar'), if in_main
...
eststo clear
global controls $covs_base
ivreghdfe St_percent_fox (St_cons_fox_msnbc=St_instr_fox_msnbc) $controls, absorb(fips_state) partial($controls) cluster(`clustvar'), if in_main
estadd scalar ftest=e(widstat)
eststo
global controls $covs_base $channel
ivreghdfe St_percent_fox (St_cons_fox_msnbc=St_instr_fox_msnbc) $controls, absorb(fips_state) partial($controls) cluster(`clustvar'), if in_main
...
eststo clear
global controls $covs_base
ivreghdfe St_percent_fox (St_rtgxx_county_fxnc=St_pos_fxnc_county_repl) $controls, absorb(fips_state) partial($controls) cluster(`clustvar'), if in_main
estadd scalar ftest=e(widstat)
eststo
global controls $covs_base $channel
ivreghdfe St_percent_fox (St_rtgxx_county_fxnc=St_pos_fxnc_county_repl) $controls, absorb(fips_state) partial($controls) cluster(`clustvar'), if in_main
...
save "data/alternative_matching_counties_to_compare.dta", replace

**************************************************
* Main table with reduced form and 2SLS, with circ data
**************************************************

* Add IPCSR circulation data
drop _merge
sort source
merge source using "data/crosswalk_newslibrary_icpsr.dta", nokeep
tab _merge
drop dup_ipcsr _merge
sort source_icpsr
merge m:1 source_icpsr using "data/icpsr_2004v2.dta", keepusing(circ)   
keep if _merge==3
